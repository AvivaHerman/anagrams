...
object Anagrams {
  ...
  /** Converts the word into its character occurence list.
    ...
    */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.groupBy(identity).mapValues(s => s.length).toList.sortBy(_._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    case Nil => List(Nil)
    case (c, i) :: Nil => List() :: (1 to i).map(j => List((c, j))).toList
    case (c, i) :: xs => {
